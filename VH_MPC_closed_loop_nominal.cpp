...
int main() {
	...
	printf("Setup nominal (%i):  %f\n", return_val, setup_time_nominal);
	...
	for (int N1 = 0; N1 < 100; N1++){
	    ...
	    if (iter!=0){
		nominal_mpc.compute_warm_start_shifted(nominal_mpc.Nprev, nominal_mpc.Nmin,
						       primal_solution, dual_solution);
	    }
	    else {
		nominal_mpc.disable_warm_start();
	    ...
	    return_val = nominal_mpc.solve_variable_horizon(Ax0, N1, 
							    primal_solution, dual_solution, 
							    results);
	    ...
	    printf("%i   %4i   %5i   %7.6f  %7.6f  %7.6f  %7.6f  %6.3f    %3i  %3i\n ",
		   ...
		   nominal_mpc.Nmin, nominal_mpc.deltaN);
	    ...
	    // STEP IN SIMULATION
	    for (int j=0;j<nu;j++) input(j) = nominal_mpc.optimal_trajectory[j][0];
	    quad.step(input);
